...
export async function createCroppedImage(source: string, crop: Area): Promise<string> {
  ...
  context.fillStyle = "#061c46"; context.fillRect(0, 0, 700, 700);
  context.drawImage(image, crop.x, crop.y, crop.width, crop.height, 0, 0, 700, 700);
  return canvas.toDataURL("image/jpeg", 0.94);
}

export function PhotoCropper({ source, onChange, onCancel }: { source: string; onChange: (area: Area) => void; onCancel: () => void }) {
  const [crop, setCrop] = useState<Point>({ x: 0, y: 0 });
  const [zoom, setZoom] = useState(1);
  const [cropPixels, setCropPixels] = useState<Area | null>(null);
  const complete = useCallback((_: Area, pixels: Area) => { setCropPixels(pixels); onChange(pixels); }, [onChange]);
  ...
  return (
    <div className="crop-shell">
      ...
      <div className="crop-controls">
        <label htmlFor="photo-zoom">Zoom</label><input id="photo-zoom" type="range" min={1} max={3} step={0.05} value={zoom} onChange={(e) => setZoom(Number(e.target.value))} />
        <button type="button" onClick={reset}><RotateCcw size={16} /> Reset</button>
        <button type="button" disabled={!cropPixels} onClick={() => cropPixels && onChange(cropPixels)}>Use crop</button>
